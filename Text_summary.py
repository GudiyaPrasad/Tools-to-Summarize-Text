...
def summarizer(rawdocs):
    ...
    #print(stopwords)
    nlp = spacy.load('en_core_web_ sm')
    doc = nlp(rawdocs)
    #print(doc)
    tokens = [tokens.text for token in doc]
    #print(tokens)
    word_freq = {}  # or some other initialization based on your code
    for word in doc:
...
#print(select_len)
    summary = nlargest(select_len, sent_scores, keys=sent_scores.get)
#print(summary)
...
def my_function():
    # some code
    return summary, docs, len(rawdocs.split(' ')), len(summary.split(' '))
